...
# مستندات کلاس WeightController

کلاس **WeightController** در پروژه‌ی **WeighingService** مسئول مدیریت درخواست‌های مرتبط با وزن از طریق API است. این کلاس اطلاعات وزن جاری را به همراه نسخه نرم‌افزار ارائه می‌دهد.

## فهرست محتوا

- [پکیج‌های مورد استفاده](#پکیج‌های-مورد-استفاده)
- [متدها](#متدها)
  - [سازنده WeightController](#سازنده-weightcontroller)
  - [Current](#متد-current)
...
- `System.Collections.Generic`: برای استفاده از انواع عمومی لیست‌ها و مجموعه‌ها.
- `System.Linq`: برای انجام عملیات مرتبط با کوئری و مدیریت داده‌ها.
- `System.Reflection`: برای دسترسی به اطلاعات متاداده‌ای اسمبلی.
- `System.Text`: برای کار با رشته‌ها.
- `System.Threading.Tasks`: برای مدیریت عملیات ناهمگام.
- `System.Web.Http`: برای ساخت APIهای مبتنی بر HTTP.
- `WeighingService.Constants`: برای استفاده از ثابت‌ها و مقادیر تعریف‌شده در پروژه.
- `WeighingService.Models`: برای مدیریت مدل‌های داده‌ای مرتبط با وزن.
- `WeighingService.Services`: برای استفاده از سرویس‌های موجود در پروژه.

## متدها

### سازنده WeightController

```csharp
public WeightController()
```

این متد سازنده، کلاس `WeightController` را مقداردهی می‌کند. در این متد، نمونه‌ای از `WeightService` بارگذاری می‌شود که برای مدیریت و ارائه اطلاعات وزن استفاده می‌شود.

### متد Current

```csharp
[HttpPost]
public IHttpActionResult Current(CurrentWeightRequest request)
```

این متد برای دریافت وزن جاری استفاده می‌شود. ابتدا نسخه نرم‌افزار را از طریق اسمبلی جاری دریافت کرده و سپس اطلاعات وزن از سرویس `WeightService` فراخوانی می‌شود. در صورت موفقیت‌آمیز بودن عملیات، وزن جاری به همراه نسخه نرم‌افزار و کد وضعیت 200 (OK) بازگردانده می‌شود.

اگر وزن در دسترس نباشد (مثلاً به دلیل گذشتن مدت‌زمان مجاز)، پیامی با کد خطای 404 (Not Found) و وزن صفر بازگردانده می‌شود. در صورت بروز خطا در عملیات، یک پیام خطا با کد خطای داخلی سرور (Internal Server Error) به همراه پیغام خطا بازگردانده می‌شود.
